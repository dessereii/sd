...
def student_view_records():
    ...
    if not student_id:
        ...
        return redirect(url_for('student_view_records'))
...
@app.route('/admin_quiz_export_csv')
def admin_quiz_export_csv():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
        SELECT student_id, submission_date,
...
@app.route('/admin_quiz_export_pdf')
def admin_quiz_export_pdf():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
        SELECT student_id, submission_date,
